...
XOPNumberOfBytesInFile(XOP_FILE_REF fileRef, UInt32* numBytesPtr)
{
	...
	originalPos = (SInt32)ftell(fileRef);
	if (fseek(fileRef, 0, SEEK_END) != 0)
		return FILE_POS_ERROR;
	*numBytesPtr = (SInt32)ftell(fileRef);
...
/*	GetFullPathFromSymbolicPathAndFilePath(symbolicPathName, filePath, fullFilePath)
	...
	Thread Safety: GetFullPathFromSymbolicPathAndFilePath is not thread-safe
	because it calls GetPathInfo2 which is not thread-safe.
*/
int
