...
#define FEATURE_NOT_AVAIL 186		// "This feature is not available or is invalid."
...
#define MOVIE_FRAME_ERR			 221	// "An error was encountered processing a movie frame."
...
#define MDFUNCFIT_IND_VAR_MISMATCH 487		// "The data wave must have one dimension for each independent variable."
...
#define NOSUBRANGECATWAVE 726				// "Subranged category waves are not currently supported."
...
#define ILLEGAL_FIELD_FOR_FBIN 961				// "No legal (numeric) fields present after last illegal field (String, NVAR etc.) in structure. I.e. no data available for storage."
...
#define SUBRANGE_DLOG_INC_TOO_BIG 1082			// "This increment causes the range to exceed the wave dimension size. It must be less than ^3."
...
#define MOVIE_END	1256							// "End of movie file."
#define MOVIE_NO_CODEC	1257						// "No suitable transform was found to encode or decode movie content."
#define MULTITHREAD_INVALID	1258					// "MultiThread is not valid here."
#define CANT_TARGET_ROOT_FOLDER 1259				// "This operation does not permit targeting the root data folder. Target a child folder."
...
enum MoreOperationErrorCodes {
	...
	// Reserved for Igor7
	LINEFIT_TOOMANYHOLDS,								// "Line fit does not support holding both a and b."
	INCOMPATIBLE_TABLE_CLIPBOARD_DATA,					// "The binary table clipboard data is incompatible with this version of Igor."
	CANT_WRITE_OVER_2GB_TO_IBW_FILE,					// "The .ibw binary wave format does not support waves larger than 2GB. Use the SaveData operation to write to a .pxp file instead."
	EXPECTED_TOPLEVELWINDOWOREXTERIORPANEL,				// "Expected the name of a top-level window or an exterior panel window."
	...
	PATH_TOO_LONG_FOR_OLD_XOP,							// "The path is too long for this XOP which is limited to 400-byte paths"
	...
	kExpect1DRealWave,									// "Expected 1d real numeric wave.
	kBadWindowFunction,									// "Bad window function specification."
    kWaveSelectionsMustBeDistinct,                      // "Wave selections must be distinct."
	kInsufficientScales,								// "Insufficient number of scales."
	kBadScalingWaveSpecification,						// "Bad scaling wave specification".
	kBadPaddingRequest,									// "The requested padding conflicts with segment size"
	kSegmentSizeTooLarge,								// "The requested segment size is too large."

	GRAPH_PANEL_EXT_PANEL_TABLE_GIZMO_ONLY,				// "Expected graph, panel, exterior panel, or Gizmo name."

	TWFA_CANT_OVERWRITE_TEXTWAVE_WITH_NUMERIC,			// "Your tick values wave cannot overwrite a non-numeric wave."
	TWFA_CANT_OVERWRITE_NONTEXT_WITH_TEXT,				// "Your tick labels wave cannot overwrite a non-text wave."

	GIZMO_EXTERNAL_PANEL_ONLY,							// "A gizmo window can host external panels only"

	// for new (IP8) dimension labels as category plot labels feature
	EXPECT_TEXT_WAVE_OR_LABELSKW,							// "Expected a text wave name or the \'_labels_\' keyword."
	NO_TEXTWAVE_FOR_NUMERIC_AXIS,						// "Cannot use a text wave for a trace plotted on a numeric axis."
	NO_NUMERIC_WAVE_FOR_CATEGORY_AXIS,					// "Cannot use a numeric wave with a category axis."
	
	XOP_MEMORY_INCOMPATIBLE,	// "The XOP is incompatible with this version of Igor. It must be recompiled to use WM memory XOPSupport routines."

	// slider repeat keyword errors
	REPEAT_RATE_GT_ZERO,								// "Slider repeat rate must be greater than zero."
	REPEAT_STYLE_ERR,									// "Slider repeat style must be zero, one or two."

	BOXPLOT_ONEMCWAVE,									// "A box plot can accept one 2D wave, or multiple 1D waves. You cannot mix 1D and 2D waves."
	VIOLINPLOT_ONEMCWAVE,								// "A violin plot can accept one 2D wave, or multiple 1D waves. You cannot mix 1D and 2D waves."
	BOXPLOT_1D_XWAVE,									// "The X wave for a box plot must be 1D (single column)."
	VIOLINPLOT_1D_XWAVE,								// "The X wave for a violin plot must be 1D (single column)."
	BOXPLOT_XWAVE_REAL_OR_TEXT,							// "The X wave for a box plot must be either a text wave or a real numeric wave."
	VIOLINPLOT_XWAVE_REAL_OR_TEXT,						// "The X wave for a violin plot must be either a text wave or a real numeric wave."
	BOXVIOLINPLOT_XWAVE_DIMENSIONMISMATCH,				// "The number of rows in the X wave must match the number of columns in the data waves."
	BOXVIOLINPLOT_XWAVE_NDATASETSMISMATCH,				// "The number of rows in the X wave must be greater than or equal to the number of 1D data waves."
	MODBOXPLOT_EXPECTEDBOXTRACE,						// "Expected a box plot trace."
	MODVIOLINPLOT_EXPECTEDVIOLINTRACE,					// "Expected a violin plot trace."
	MODBOXPLOT_ShowData_BadParam,						// "ShowData is expected to be None, All, Outliers, or FarOutliers."
	BOXVIOLINPLOT_BAD_TRACENAME,						// "Bad user-specified trace name."	for /TN flag
	BOXPLOT_NO_ERRORBAR,								// "Cannot add error bars to a box plot trace."
	VIOLINPLOT_NO_ERRORBAR,								// "Cannot add error bars to a violin plot trace."
	NOREPLACEYINBOXORVIOLINPLOT,						// "Cannot replace the Y data in a box plot or violin plot trace."

	kExpected1DWave,                                    // "Expected 1D numeric wave."
	kSegmentTooLarge,									// "Segment size can't be larger than the source."
	kPaddedSizeTooSmall,								// "Padded size must be larger than segment size."
	kBadDBScaling,										// "Mismatch between output type and dB scaling."
	kParamOutOfRange,									// "Parameter is out of range."
	kCameraNotReady,									// "Camera not ready for capture."

	DRAWSCRAP_VERSION_ERROR,							// "The clipboard contains Igor draw or control data that is incompatible with this version of Igor."

    kLibraryFailedToInitialize,                         // "Library failed to initialize."
    kTransformCommitErr,                                // "Error on commiting transform."
    kTransformFreeErr,                                  // "Error on freeing transform."
    kTransformExecutionErr,                             // "Transform execution error."
	kWantsU16Data,										// "Expected unsigned short wave."
    kDoesNotSupportInt64,                               // "Does not support Int64."
	kMissingAlphaSpecification,							// "Missing alpha specification."
	kExpectedSPWave,									// "Expected single-precision floating point wave."
    kBadDimensionSpecification,                         // "Bad dimension specification."
	PEAK_CTR_RANGE,										// "A peak center was less than 0 or greater than the number of points in the peak wave - 1"
	BASE_PEAK_DIFFERENT_LEN,							// "Baseline wave has different number of points than peak wave"
	X_PEAK_DIFFERENT_LEN,								// "X wave has different number of points than peak wave"
	POLARITY_RANGE,										// "Polarity parameter must be 1 (positive) or 2 (negative)"

	SLEEP_PROGEXPECTEDNAME,								// "The PROG flag requires at least one of the two strings to be non-empty."
	
	REQUIRES_NO_RETURN_TYPE,							// "A function that uses multiple return syntax is required."
	REQUIRES_PASS_BY_REF_PARAMETER,						// "Requires pass-by-reference parameter."
	REQUIRES_PASS_BY_VALUE_PARAMETER,					// "Requires pass-by-value parameter."
	REQUIRES_WAVE_WAVE_PARAMETER,						// "Requires WAVE/WAVE parameter."
	REQUIRES_DFREF_WAVE_PARAMETER,						// "Requires WAVE/DF parameter."
	kBadAPMToken,										// "Bad APM Token."

	MDS_NO_CUSTOMIZE_AT_POINT,							// "Customize at a point is not yet supported by box and violin plot traces."
	THIS_COMMAND_FUNCSONLY,								// "This command may only be used in user-defined functions."
    // End of Igor8 Error Codes ]
};
...
enum LibcurlErrorCodes {
	...
	kLibcurl_CURLE_HTTP2_STREAM,			/* "Stream error in HTTP/2 framing layer. */
};
...
enum {
	...
	URL_MISSING_PROTOCOL,				// "The URL must include \"http://\", \"https://\", or \"ftp://\"."
	URL_MISSING_HTTP,					// "The URL must include \"http://\" or \"https://\" (e.g., <http://www.wavemetrics.com>)."
	...
	TEXT_CONTAINS_CONTROL_CHARACTER,		// "The text contains one or more control characters"
};
