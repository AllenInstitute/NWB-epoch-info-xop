...
/*	UniqueName2(nameSpaceCode, baseName, finalName, suffixNumPtr)
	...
	If your goal is to generate a valid object name that you can use to create a data object,
	consider using CreateValidDataObjectName instead of this routine.
	...
	You can call it from a thread created by Igor but not from a private thread that you created yourself.
	In Igor Pro 8.00 or later, UniqueName2 returns an error if called from an Igor pre-emptive thread
	if nameSpaceCode is other than MAIN_NAME_SPACE, DATAFOLDER_NAME_SPACE, or PATHS_NAME_SPACE.
*/
int
...
/*	CheckName(dataFolderH, objectType, name)
	...
	objectType is one of the following which are defined in IgorXOP.h:
		...
		PATH_OBJECT						Threadsafe in Igor Pro 8.00 or later
	...
	You can call it from a thread created by Igor but not from a private thread that you created yourself.
	In Igor Pro 8.00 or later, CheckName returns an error if called from an Igor pre-emptive thread
	if objectType is other than WAVE_OBJECT, VAR_OBJECT, STR_OBJECT, DATAFOLDER_OBJECT, or PATH_OBJECT.
*/
int
	...
	int* nameChangedPtr, int* doOverwritePtr)
{
	...
	*nameChangedPtr = *doOverwritePtr = 0;
	...
	if (objectType<FIRST_OBJECT_TYPE || objectType>LAST_OBJECT_TYPE) {
		// HR, 2017-04-04, XOP Toolkit 7.02: Previously this would loop forever if objectType was invalid
		err = BAD_OBJECT_TYPE;
		return err;
	}
					...
					while(1) {
						...
						*suffixNumPtr += 1;
						...
						if (iterations > 1000000) {
							/*	HR, 2017-04-04, XOP Toolkit 7.02: Previously this would loop forever if objectType
								was invalid. Now this error is caught above and we should never get here.
							*/
							err = GENERAL_BAD_VIBS;
							goto done;
						}
					}
				}
...
/*	GetPathInfo2(pathName, fullDirPath)
	...
	Thread Safety: GetPathInfo2 is thread-safe only when running with Igor Pro 8.00 or later.
*/
int
...
/*	GetFunctionInfo(name, fip)
	...
	GetFunctionInfo returns via fip->returnType one of the following codes:
		...
		FV_NORETURN_TYPE:	Function returns multiple values and has no direct result
...
/*	CheckFunctionForm(fip, requiredNumParameters, requiredParameterTypes, badParameterNumberPtr, returnType)
	...
	For an overview, see Calling User-Defined and External Functions in the XOP Toolkit manual.
	
	Return values from functions that use multiple return syntax are treated like pass-by-reference
	input parameters. See Calling User-Defined Functions That Return Multiple Results in the XOP
	...
	values above. For example:
		...
		DATAFOLDER_TYPE | FV_REF_TYPE		// Requires Igor Pro 8.00 or later
		WAVE_TYPE | FV_REF_TYPE				// Requires Igor Pro 8.00 or later
	
	Pass-by-reference is applicable to numeric and string parameters and, with Igor Pro 8.00
	or later, to data folder reference and wave reference parameters.
	
	When you call CheckFunctionForm, pass-by-reference wave parameters must indicate
	a specific type of wave:
		Parameter Type Code							Parameter Type					Matches
		WAVE_TYPE | FV_REF_TYPE						Real numeric wave				WAVE w
		WAVE_TYPE | NT_CMPLX | FV_REF_TYPE			Complex numeric wave			WAVE cw
		TEXT_WAVE_TYPE | FV_REF_TYPE				Text wave						WAVE tw
	...
	If the function is an external function which takes a wave parameter,
	there is no way to know if the external function expects a numeric wave, text wave,
	data folder reference wave, or wave reference wave. Consequently, CheckFunctionForm
	does not distinguish between these types of waves for external functions. The external
	function itself is supposed to check the type of the wave passed in at runtime and return
...
/*	CallFunction(fip, parameters, resultPtr)
	...
	However, if the function result type is FV_NORETURN_TYPE, this means that the function
	uses multiple return syntax and has no direct result. In this case, and only
	in this case, CallFunction ignores resultPtr and you should pass nullptr for it.
